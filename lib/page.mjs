...
const footerTemplate = page => `${dumpTemplates()}<script>window.page=${JSON.stringify(page)}</script><script>window.cache=${db.exportCache()}</script><script>window.imports=${JSON.stringify(Object.keys(imports))}</script>${footer}</body></html>`
...
export async function createRenderThread(req, res) {
    ...
    const generator = new Generator({
        ...
        env: [process.env.VERCEL_ENV === 'development' ? 'development' : 'production', 'browser', 'module'],
    });
    ...
    stream.on('end', async () => {
        ...
        const output = Buffer.concat(chunks) + footerTemplate(page)
        return res.end(await generator.htmlGenerate(output, {esModuleShims: true}))
    })
